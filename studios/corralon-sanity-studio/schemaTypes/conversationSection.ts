...
const TConversationSection = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      title: 'Imagen',
      type: 'image',
      description: 'Imagen que se muestra en el centro de la sección',
      options: {
        hotspot: true,
      },
      fields: [
     ...
     defineField({
      ...
      type: 'string',
      description: 'Título principal que aparece al inicio de la conversación',
      validation: (Rule) => Rule.required(),
    }),
    defineField({
      name: 'content',
      ...
      type: 'richText',
      description:
        'Contenido principal que aparece después de la imagen con formato de texto enriquecido',
      validation: (Rule) => Rule.required(),
    }),
  ],
  preview: {
    ...
    prepare({title, image, content}: {title: string; image: any; content: Array<any>}) {
      ...
      return {
        title: title || 'Sección de Conversación',
...
export default TConversationSection